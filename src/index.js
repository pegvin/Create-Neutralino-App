...
async function validateRepository(userAndRepo) {
    ...
    const data = await (
        await fetch(`https://api.github.com/repos/${userAndRepo}`)
    ).json();
    // The following line will always default to false because the data.message attribute doesn't
    // exist when the repo is found.
    if (data.message && data.message.toLowerCase() === 'not found') {
        // If API returned "not found"
        return 'REPO_NOT_FOUND';
    } else {
        // Get the information about "neutralino.config.json" from the github repo.
        const content = await (
            await fetch(
                `https://api.github.com/repos/${userAndRepo}/contents/neutralino.config.json`
            )
        ).json();
        if (content.message && content.message.toLowerCase() == 'not found') {
            // if API returned "not found"
...
function renameTemplate(appName, appPath, template) {
    if (fs.existsSync(path.join(appPath, 'neutralino.config.json'))) {
        ...
        var configJSON = fs.readFileSync(
            path.join(appPath, 'neutralino.config.json'),
            {
                encoding: 'utf-8',
            }
        );
 ...
 * @param {String} template Name Of the Template User Selected.
 * @param {String} appName Name Of the App User Specified.
 * @param {String} branchName Name of the branch that the template is found in.
 */
async function downloadTemplate(
    template = templateUrls['default-neu'].src,
    appName,
    branchName
) {
    // Create A Spinner And Start it.
    let spinnerInstance = nanospinner
        .createSpinner(`Downloading Template`)
        .start();
    ...
    try {
        ...
        const response = await fetch(
            `https://github.com/${template}/archive/${branchName}.zip`
        );
        if (!response.ok)
        ...
        for (let i = 0; i < files.length; i++) {
            ...
            if (!fileName.endsWith('/')) {
                // if it is a file then read it's content and write into it.
                let fileData = await zipFileContents
                    .file(fileName)
                    .async('nodebuffer');
                fs.writeFileSync(path.resolve(files[i]), fileData);
            } else {
                // if filename is a directory then make that directory
...
async function getFrontendLib() {
    ...
    if (answers.frontend_lib.toLowerCase() == 'none') {
        ...
        // else return the template name in lowercase.
...
if (process.argv.length > 2) {
    let userNameAndRepo = process.argv[2];
    if (process.argv.length > 3) {
        branchName = process.argv[3];
    }
    // check if the repository path format is valid, i.e. "USERNAME/REPO"
    if (userNameAndRepo.split('/').length >= 2) {
        // check if the repository passed is a valid repository
        let validRepo = await validateRepository(userNameAndRepo);

        if (validRepo === true) {
            frontendLib = userNameAndRepo.split('/')[1];
            templateURL = userNameAndRepo;
    ...
    // if no argument is passed then get the input from user.
...
await downloadTemplate(templateURL, appName, branchName);
...
if (templateUrls[frontendLib]) {
    ...
    console.log(
        `We Recommend you to run these commands inside your project directory before doing anything:\n`
    );

    // Show the Recommended Commands.
    for (
        let i = 0;
        i < templateUrls[frontendLib].recomendedCommands.length;
        i++
    ) {
        console.log(
            `  ${neutralinoGradient(
                templateUrls[frontendLib].recomendedCommands[i]
            )}`
        );
    }
}
